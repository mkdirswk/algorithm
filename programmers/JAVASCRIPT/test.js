...
function solution(nums) {
    ...
    var half_nums = Math.floor(nums.length / 2);
    var copy_nums = [];

    for(i = 0; i < nums.length; i++){
        //console.log(copy_nums.indexOf(nums[i]));
        if(copy_nums.indexOf(nums[i]) == -1){
            copy_nums.push(nums[i]);
        }
    };

    if(half_nums <= copy_nums.length){
        return half_nums;
    }
    else{
